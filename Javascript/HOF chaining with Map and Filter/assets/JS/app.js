...
let products=[
    {
      ...
      "description": "Lorem ipsum dolor, consectetur adipiscing elit.",
      "availability": true
    },
    {
      "id": 2,
...
// array of only the prices of the products?

let prod=products.map(pros=>{
    return {
        prices:pros.price
    }
})
...
// generate an array of objects containing only the name and price of each product.
let prods=products.map(pros=>{
    return{
        price:pros.price,
        priceName:pros.name
    }
...
// the product array to retrieve products with prices higher than $50.


let prods50=products.filter(pros=>{
    return pros.price>50
})
cl(prods50);


//array of only products in the "Electronics" category.

let prodele=products.filter(pros=>{
    return pros.category.toLowerCase().includes("electronics");
})
cl(prodele);

//  obtain a list of product names for available products in the "Clothing" category only

let proda=products.filter(pros=>{
    return pros.category.toLowerCase().includes("clothing")
})
cl(proda);
...
// retrieve products with descriptions longer than 50 characters.
 ...
 let prodgr=products.filter(pros=>{
    return pros.description.length>50
 })
